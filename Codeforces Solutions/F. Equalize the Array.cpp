...
int main(){
    ...
    for(int ti = 1; ti <= t ; ++ti){
        ...
        for(int c = 1; c <= allfreq.back() ; ++c){
          int pos = upper_bound(allfreq.begin(), allfreq.end(), c) - allfreq.begin();
          int suf = query(N-1)-query(pos-1);
          int qtd = N-pos;
          int prefPos = upper_bound(allfreq.begin(), allfreq.end(), c-1)-allfreq.begin()-1;
          int pref = query(prefPos);
          int toRemove = pref+ suf-c*qtd;
